...
public class PasswordForget extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            StudentsBean sb = sd.getByEnroll(enrollment);

            if (currentpassword.equals(sb.getPassword()) && dob.equals(sb.getDob()) && newpassword.equals(confirmpassword)) {
                Connection conn;
                conn = ConnectionPool.ConnectDB();
                int r = 0;
                String sql = "UPDATE student SET password = '" + newpassword + "', repassword = '" + confirmpassword + "' WHERE enrollment ='" + enrollment + "' ";
                Statement stmt;
                try {
                ...
                if (r > 0) {
                    response.sendRedirect("successPasswordForget.jsp");
                }

            } else {
                response.sendRedirect("failedPasswordForget.jsp");
            }
